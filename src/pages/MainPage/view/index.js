...
const MainPage = () => {
  ...
  const fetchApi = useCallback(async () => {
    ...
    try {
      ...
      // retrieve the dropdown options from the API
  ...
  useEffect(() => {
    // To filter data based on search country and dropdown region values
    const filteredData = countries
      ?.filter((item) => {
        if (
          dropdownValue === "" ||
          dropdownValue.toLowerCase() === "All region".toLowerCase()
        ) {
          return item;
        } else if (
          item.region.toLowerCase().includes(dropdownValue.toLowerCase())
        ) {
          return item;
        } else {
          return null;
      ...
      <div className="flex flex-col items-start space-y-6 md:space-y-0 md:flex-row md:justify-between md:items-center w-full">
        ...
        <div className="flex items-center space-x-2 w-full md:w-auto">
          <AcdoDropdown
            ...
            className={`min-w-0 w-full md:w-auto md:min-w-[250px]`}
          />
          <div className="flex items-center space-x-3">
            <MdList
              size={20}
              className="text-acdo-primary dark:text-acdo-grayText"
            />
            <AcdoSwitch checked={checked} onChange={handleSwitch} />
            <MdGridView
              size={20}
              className="text-acdo-primary dark:text-acdo-grayText"
            />
