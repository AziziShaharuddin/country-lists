        ...
        <tbody className="py-6">
          {data?.map((country, index) => {
            ...
            return (
              <tr
                key={index}
                className={`cursor-pointer dark:hover:bg-acdo-background dark:hover:text-acdo-primary hover:bg-acdo-grayLine hover:transition-all dark:text-acdo-grayLine hover:duration-200 ${data?.length !== index + 1 && "border-b"
                  }`}
              ...
              >
                ...
                <td className="py-2 px-2">
                  {populationValueConversion(country.population)}
                </td>
                ...
                <td className="py-2 px-2">
                  {country.capital &&
                    country.capital.map(
                      (cap, index) =>
                        `${cap}${country.capital?.length !== index + 1 ? ", " : ""
                        }`
                    )}
                </td>
                ...
                <td className="py-2 px-2">
                  <div className="flex space-x-2">
                    {currencies?.map((obj, objId) => (
                      <div
                        key={objId}
                        className="rounded-[4.5px] dark:bg-acdo-secondary dark:text-acdo-white dark:border-none w-auto p-1 border border-acdo-darkText bg-acdo-white  text-caption text-acdo-secondary"
                      >
                        {obj[Object.keys(obj)[0]]?.symbol} {Object.keys(obj)[0]}
                      </div>
                    ))}
                  </div>
                </td>
