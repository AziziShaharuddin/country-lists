...
const DetailPage = () => {
  ...
  const fetchApi = useCallback(async () => {
    try {
      ...
      const data = res.find((obj) => obj.name.common.toLowerCase() === name.toLowerCase())
      setData(data ? {
        ...data,
  ...
  const nativeNameArray = data?.name?.nativeName ? Object.entries(data?.name?.nativeName).map(([_, value]) => (value?.common)) : null
  const currencies = data?.currencies ? Object.entries(data?.currencies).map(([key, value]) => ({ [key]: value })) : null
  const lang = data?.languages ? Object.entries(data?.languages).map(([_, value]) => (value)) : null
  const detailsArray = [
    {
      title: 'Native name',
      value: nativeNameArray && nativeNameArray?.map((nat, index) => (`${nat}${nativeNameArray?.length !== index + 1 ? ', ' : ''}`))
    },
    {
      title: 'Population',
    ...
    {
      title: 'Capital',
      value: data?.capital && data?.capital?.map((cap, index) => (`${cap}${data.capital?.length !== index + 1 ? ', ' : ''}`))
    },
    {
      title: 'Top level domain',
      value: data?.tld && data?.tld?.map((item, index) => (`${item}${data.tld?.length !== index + 1 ? ', ' : ''}`))
    },
    {
      title: 'Currencies',
    ...
    {
      title: 'Languages',
      value: lang && lang?.map((item, index) => (`${item}${lang?.length !== index + 1 ? ', ' : ''}`))
    },
  ]
  ...
  return (
    <div className="flex flex-col items-start space-y-4 w-full">
      ...
      {
        data ? <div className="flex flex-col w-full lg:flex-row items-start h-auto space-y-4 lg:space-y-0 lg:space-x-10">
